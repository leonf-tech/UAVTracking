...
if __name__ == '__main__':
    ...
    for data_name in data_names:
        ...
        uav123_results[data_name]['gts']=[]
        for gt in gts:
            uav123_results[data_name]['gts'].append(list(gt.astype(np.int)))

        kcf_gray_preds=tracker_kcf_gray.tracking()
        uav123_results[data_name]['kcf_gray_preds']=[]
        for kcf_gray_pred in kcf_gray_preds:
            uav123_results[data_name]['kcf_gray_preds'].append(list(kcf_gray_pred.astype(np.int)))
        print('kcf gray done!')

        kcf_hog_preds=tracker_kcf_hog.tracking()
        uav123_results[data_name]['kcf_hog_preds'] = []
        for kcf_hog_pred in kcf_hog_preds:
            uav123_results[data_name]['kcf_hog_preds'].append(list(kcf_hog_pred.astype(np.int)))
        print('kcf hog done!')

        dcf_gray_preds=tracker_dcf_gray.tracking()
        uav123_results[data_name]['dcf_gray_preds'] = []
        for dcf_gray_pred in dcf_gray_preds:
            uav123_results[data_name]['dcf_gray_preds'].append(list(dcf_gray_pred.astype(np.int)))
        print('dcf gray done!')

        dcf_hog_preds=tracker_dcf_hog.tracking()
        uav123_results[data_name]['dcf_hog_preds'] = []
        for dcf_hog_pred in dcf_hog_preds:
            uav123_results[data_name]['dcf_hog_preds'].append(list(dcf_hog_pred.astype(np.int)))
        print('dcf hog done!')

        mosse_preds=tracker_mosse.tracking()
        uav123_results[data_name]['mosse'] = []
        for mosse_pred in mosse_preds:
            uav123_results[data_name]['mosse'].append(list(mosse_pred.astype(np.int)))
        print('mosse done!')

        csk_preds=tracker_csk.tracking()
        uav123_results[data_name]['csk'] = []
        for csk_pred in csk_preds:
            uav123_results[data_name]['csk'].append(list(csk_pred.astype(np.int)))
        print('csk done!')

        kcf_cn_preds=tracker_kcf_cn.tracking()
        uav123_results[data_name]['kcf_cn']=[]
        for kcf_cn_pred in kcf_cn_preds:
            uav123_results[data_name]['kcf_cn'].append(list(kcf_cn_pred.astype(np.int)))
        print('kcf_cn done!')


        # kcf_pyECO_cn_preds = tracker_kcf_pyECO_cn.tracking()
        # uav123_results[data_name]['kcf_pyECO_cn'] = []
        # for kcf_cn_pred in kcf_cn_preds:
        #     uav123_results[data_name]['kcf_pyECO_cn'].append(list(kcf_cn_pred.astype(np.int)))
        # print('kcf_pyECO_cn done!')
        #
        # kcf_pyECO_hog_preds=tracker_kcf_pyECO_hog.tracking()
        # uav123_results[data_name]['kcf_pyECO_hog']=[]
        # for kcf_pyECO_hog_pred in kcf_pyECO_hog_preds:
        #     uav123_results[data_name]['kcf_pyECO_hog'].append(list(kcf_pyECO_hog_pred.astype(np.int)))
        # print('kcf_pyECO_hog done!')

        cn_preds=tracker_cn.tracking()
        uav123_results[data_name]['cn']=[]
        for cn_pred in cn_preds:
            uav123_results[data_name]['cn'].append(list(cn_pred.astype(np.int)))
        print('cn done!')
        ...
        plt.legend()
        plt.savefig('../results/UAV123_cftrackers/'+data_name+'_precision.jpg')
        plt.clf()
        ...
        plt.ylabel('success')
        plt.legend()
        plt.savefig('../results/UAV123_cftrackers/'+data_name+'_success.jpg')
        plt.clf()

    json_content = json.dumps(uav123_results, default=str)
    f = open('../results/UAV123_cftrackers/UAV123_results.json', 'w')
