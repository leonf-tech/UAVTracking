...
from .yolov3.detect_new import  yolo_detection
from .detectron2_detector.detetron2_test import faster_rcnn_detection
...
# parent_dir_path = os.path.dirname(os.path.dirname(module_path))
...
class ReIDKCF(BaseCF):
    ...
    def update(self,current_frame,idx=0,image_path=None,vis=False,):
        '''
        ...
        apce = APCE(self.score)
        psr = PSR(self.score)
        w1 = 0.5
        w2 = 0.5

        weighted_apce_psr = w1*apce+w2*psr
        # print("#frame:{},apce:{},psr:{},weighted_value:{}".format(idx, apce, psr,weighted_apce_psr))
        reid_threshold = 14.95
        # print("weighted_apce_psr")
        # print(weighted_apce_psr)
        if(weighted_apce_psr<reid_threshold or self._center==None):
            #do yolo detection
            #
            # print("enter yolo detection branch")
            # print(image_path)
            """
            ../dataset/UAV123/car12/img/000018.jpg
            """
            name = image_path.split("/")[-3]+"_"+image_path.split("/")[-1]
            #car12_000018.jpg

            #TODO: detect API YOLOdetect
            # write_to_path = os.path.join(dir_path,"yolov3/data/to_detect/"+name)
            # print("write_to_path:{}".format(write_to_path))
            # cv2.imwrite(write_to_path,current_frame)
            # self._center  = yolo_detection(write_to_path)#TODO:进行yolo检测,传输真正的图像
            write_to_path = os.path.join(dir_path, "detectron2_detector/to_detect/" + name)
            print("write_to_path:{}".format(write_to_path))
            cv2.imwrite(write_to_path, current_frame)
            self._center = faster_rcnn_detection(name)  # TODO:进行yolo检测,传输真正的图像
            # print(self._center)
            if(self._center == None):
                return None
            else:#detect以后，初始化model
                if self.features == 'color' or self.features == 'gray':
                    new_x = self._crop(current_frame, self._center, (self.w, self.h))
                elif self.features == 'hog':
                    new_x = self._crop(current_frame, self._center, (self.w, self.h))
                    new_x = cv2.resize(new_x,
                                       (self.window_size[0] * self.cell_size, self.window_size[1] * self.cell_size))
                ...
                self.xf = fft2(self._get_windowed(new_x, self._window))
                self.alphaf = self._training(self.xf, self.yf)
            ...
            if curr[0]+1>self.window_size[1]/2:
                ...
                dy=curr[0]
            if curr[1]+1>self.wiqndow_size[0]/2:
                # delta x
